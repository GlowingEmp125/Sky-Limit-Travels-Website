...
export default function DestinationForm({ initialData, isEditing = false }: DestinationFormProps) {
  ...
  const isDestinationValid = (destination: any) => {
    if (!destination) return false;

    const { destinationImage, ...rest } = destination;

    return Object.values(rest).every((value) => value !== "" && value !== null && value !== undefined);
  };

  // Landing Page Tips handlers
  const addLandingPageTip = () => {

    const landingPageTips = formData.landingPageTips;
    const lastTip = landingPageTips[landingPageTips.length - 1];

    if (!isDestinationValid(lastTip)) {
      alert("Please complete the last Landing Page Tip first.");
      return;
    }
